...
@pytest.mark.asyncio
async def test_refresh_right_path(client, test_uow):
    login = 'not_registered_login'
    password = 'password'
    await test_uow.user_repository.add_with_ignore_conflict(
        value={
    ...
    UserRepository.update_by_id = AsyncMock(return_value=None)

    refresh_response = client.post(f"/auth/refresh")
    assert refresh_response.status_code == HTTPStatus.OK
    assert UserRepository.update_by_id.call_count == 1
    kwargs = UserRepository.update_by_id.call_args.kwargs
